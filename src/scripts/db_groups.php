<?php
require_once "db_utils.php";
require_once "db_users.php";

function create_group($db, $name, $course, $subject, $max_members, $description, $is_public,
    $owner_email, $password = null) {
    $owner_id = get_user_id_by_email($db, $owner_email);

    $hash = ($password) ? password_hash($password, PASSWORD_DEFAULT) : null;
    // Se $is_public è true diventa 't', altrimenti 'f'.
    // Questo impedisce che arrivi una stringa vuota "" al database, che mi ha creato MOLTI errori :(.
    $bool_public = ($is_public) ? 't' : 'f';

    $sql = "INSERT INTO groups(name, course, subject, max_members, description, is_public, owner_id, group_pass_hash) 
            VALUES($1, $2, $3, $4, $5, $6, $7, $8)";

    pg_query_params($db, $sql, array($name, $course, $subject,
        $max_members, $description, $bool_public, $owner_id, $hash));

    return true;
}

function check_group_password($db, $group_id, $password_input) {
    $sql = "SELECT group_pass_hash FROM groups WHERE id = $1";
    $res = pg_query_params($db, $sql, array($group_id));
    
    if ($row = pg_fetch_assoc($res)) {
        // Se l'hash è null nel DB, il gruppo è tecnicamente "aperto".
        if ($row['group_pass_hash'] === null) return true; 
        
        // Verifica hash
        return password_verify($password_input, $row['group_pass_hash']);
    }
    return false;
}

function get_group_with_id($db, $id) {
    $sql = "SELECT * 
            FROM groups
            WHERE id = $1";

    $res = pg_query_params($db, $sql, array($id));

    return fetch_one($res);
}

function get_groups_starting_with($db, $str) {
    $sql = "SELECT * FROM groups
            WHERE name ILIKE $1
            OR course ILIKE $1
            OR subject ILIKE $1";

    $res = pg_query_params($db, $sql, array($str . "%"));

    return fetch_all($res);
}

function get_group_id_by_name($db, $name) {
    $sql = "SELECT id
            FROM groups
            WHERE name = $1";

    $res = pg_query_params($db, $sql, array($name));

    return fetch_value($res);
}

function add_user_to_group($db, $group_name, $user_email) {
    $group_id = get_group_id_by_name($db, $group_name);
    $user_id = get_user_id_by_email($db, $user_email);

    $sql = "INSERT INTO group_participants(group_id, user_id)
            VALUES($1, $2)";

    $res = pg_query_params($db, $sql, array($group_id, $user_id));

    if ($res) {
        $sql_update = "UPDATE groups 
                       SET curr_members = curr_members + 1 
                       WHERE id = $1";
        pg_query_params($db, $sql_update, array($group_id));
    }
}

function remove_user_from_group($db, $group_id, $user_id) {
    $sql = "DELETE FROM group_participants WHERE group_id = $1 AND user_id = $2";
    $res = pg_query_params($db, $sql, array($group_id, $user_id));

    if ($res) {
        $update_sql = "UPDATE groups SET curr_members = curr_members - 1 WHERE id = $1";
        pg_query_params($db, $update_sql, array($group_id));
        return true;
    }
    return false;
}

function get_users_in_group($db, $group_name) {
    $group_id = get_group_id_by_name($db, $group_name);
    
    $sql = "SELECT u.id, u.name, u.surname, u.email 
            FROM group_participants gp
            JOIN users u ON gp.user_id = u.id
            WHERE gp.group_id = $1"; 
    
    $res = pg_query_params($db, $sql, array($group_id));
    return fetch_all($res);
}

function delete_group($db, $group_id) {
    $sql_participants = "DELETE FROM group_participants 
                         WHERE group_id = $1";
    
    pg_query_params($db, $sql_participants, array($group_id));

    $sql_group = "DELETE FROM groups 
                  WHERE id = $1";

    $res = pg_query_params($db, $sql_group, array($group_id));

    return ($res !== false);
}

function get_groups_by_user($db, $email) {
    $user_id = get_user_id_by_email($db, $email);

    $sql = "SELECT * FROM groups
            WHERE id IN (
                SELECT group_id
                FROM group_participants
                WHERE user_id = $1
                )";

    $res = pg_query_params($db, $sql, array($user_id));

    return fetch_all($res);
}

?>
